...
const getAxisLabel = (formData) => formData?.orientation === 'horizontal'
...
export const xAxisMixin = {
    label: (state) => getAxisLabel(state?.form_data).label,
    multi: false,
    description: (state) => getAxisLabel(state?.form_data).description,
    validators: [validateNonEmpty],
    initialValue: (control, state) => {
        if (state?.form_data?.granularity_sqla &&
...
export const datePickerInAdhocFilterMixin = {
    initialValue: (control, state) => {
        ...
        if (state?.controls?.time_range?.value ||
            ensureIsArray(control.value).findIndex((flt) => flt?.operator === 'TEMPORAL_RANGE') > -1) {
            return undefined;
        }
        ...
        const temporalColumn = state?.datasource &&
